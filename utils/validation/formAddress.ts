...
export const addressSchema = yup.object({
  ...
  phoneNumber: yup
    ...
    .matches(
      ...
      {
        message: "must start with + and can only contains digits",
        excludeEmptyString: true,
      }
    )
    .required("must start with + and can only contains digits"),
  firstAddressLine: yup
    ...
    .required("must be at least 2 characters long"),
  secondAddressLine: yup.string().matches(/^([A-Za-z0-9]+[\-'\s]?)$/, {
    message: "only special characters - ' are allowed",
    excludeEmptyString: true,
  }),
  city: yup
